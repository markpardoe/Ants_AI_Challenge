...
class InfluenceMap < Array2D
	...
	def initialize(row, cols, tilemap, default = 0)
		super(row, cols, default)
		@tiles = tilemap
	end
	...
	def add_influence_block(row, col, val, radius)
		circ = (radius *2)-1
		checked = Array2D.new(circ, circ, false)
	...
	def add_influence_linear(row, col, val,radius)
		circ = (radius *2)-1
		checked = Array2D.new(circ, circ, false)
		...
		(0..radius-1).each do |distance|
			...
			nodes.each do |point|
				...
				self[curRow,curCol] += val - distance 	#update tile value
